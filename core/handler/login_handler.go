...
func (h *LoginHandler) Login(c *gin.Context) {
	if c.Request.Method != http.MethodPost {
		response.ErrorMessage(c, http.StatusMethodNotAllowed, "Method not allowed")
		return
	}
	...
	if err := c.ShouldBindJSON(&body); err != nil {
		response.ErrorMessage(c, http.StatusBadRequest, "Invalid request body")
		return
	}

	token, err := h.service.Login(body.Password)
	if errors.Is(err, service.ErrAdminNotConfigured) {
		response.ErrorMessage(c, http.StatusServiceUnavailable, "ADMIN_PASSWORD is not configured on the server")
		return
	}
	if errors.Is(err, service.ErrInvalidPassword) {
		response.ErrorMessage(c, http.StatusUnauthorized, "Invalid password")
		return
	}
	if err != nil {
